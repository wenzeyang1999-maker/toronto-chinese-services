...
- [x] `review_replies` 表（reply_id, review_id, replier_id, content, created_at）
- [x] 服务详情页评价下方加「回复」入口（仅服务商可见）
- [x] 回复显示在评价下方，带「商家回复」标签 + 编辑/删除
- [ ] ProviderProfile 评价也展示回复（待做，目前仅 ServiceDetail 有）

### C. 关注服务商 ✅ 已完成
**问题：** 没有粉丝机制，服务商发新内容没人知道，用户找不回之前看过的好服务商。
- [x] `follows` 表（follower_id, provider_id, created_at，UNIQUE）
- [x] ProviderProfile 加「关注」按钮 + 粉丝数显示
- [x] Profile → 新增「我的关注」section（已关注的服务商列表）
- [ ] 关注后有新帖子时推送通知（依赖邮件通知）
...
- [x] README 全面更新
- [x] **B — 评价回复**：review_replies 表 + 服务详情页商家回复 UI（编辑/删除）
- [x] **C — 关注服务商**：follows 表 + followsStore + FollowButton + FollowsSection + 粉丝数
...
| `saves_schema.sql` | ✅ |
| `views_schema.sql` | ✅ |
| `review_replies_schema.sql` | ⏳ 待执行 |
| `follows_schema.sql` | ⏳ 待执行 |
