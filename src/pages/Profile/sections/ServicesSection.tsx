...
const TABS: { key: Tab; label: string; icon: React.ReactNode }[] = [
  { key: 'services',   label: '商家推广', icon: <Wrench      size={14} /> },
...
export default function ServicesSection() {
  ...
  async function markSecondhandSold(item: SecondhandItem) {
    ...
    const { error } = await supabase.from('secondhand')
      .update({ is_sold: true, sold_at: new Date().toISOString() }).eq('id', item.id)
    if (error) { toast(`标记失败：${error.message}`, 'error'); return }
    setSecondhand(prev => prev.map(x => x.id === item.id ? { ...x, is_sold: true } : x))
    toast('已标记为已售出', 'success')
  }
  ...
  if (loading) return (
    <div className="flex-1 space-y-3 px-4 py-5 max-w-md mx-auto w-full">
      {[1, 2, 3].map((i) => (
        <div key={i} className="bg-white rounded-2xl border border-gray-100 overflow-hidden animate-pulse flex gap-3">
        ...
        <motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.15 }}>
          ...
          {tab === 'services' && (
            <div className="space-y-3">
              {services.length === 0 && <Empty label="还没有商家推广" action={() => navigate('/post')} actionLabel="发布商家推广" />}
              <AnimatePresence>
                {services.map(svc => (
