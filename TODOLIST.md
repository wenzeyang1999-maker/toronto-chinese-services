...
平台整体 ≈ **7.8/10**(6.5 起步 → 排误报 7.0 → 多轮修复 → 会员权益落地后 7.8)。**已无 5.x/6.x**,全平台 ≥7.0。
会员方案A(金标+优先展示+免费置顶)已落地,空权益缺口闭合。剩余多为打磨细节。
...
| 区块 | 模块 | 分数 | 状态 / 一句话短板 |
|---|---|---|---|
| 发现 | 首页 Home | 7.5 | ⬆️ 首屏空状态/骨架 + **推荐区块补「猜你喜欢」标题** |
| 发现 | 分类页 Category | 7.5 | ⬆️ 无定位距离档回退好评排 + "价格从低到高"消歧义 |
| 发现 | 搜索 Search | 7.0 | ⬆️ 原 5.5;"无防抖"**误报**(gate 在提交)、技能词模糊**已修** |
| 发现 | 地图 Map/ServiceMap | 7.5 | ⬆️ 加载期显示**「地图加载中…」骨架**(不再白屏);geoAutoRequested 全局变量次要 |
| 发现 | 推荐/Banner/微件 | 7.5 | 普遍较好;Save/Follow/ViewCount **挂载抖动已修**(淡灰就绪态/占位骨架) |
| 找服务 | 服务详情 ServiceDetail | 7.5 | 文案冗长、phone/wechat 展示不一致 |
| 找服务 | 发布服务 PostService | 7.5 | ⬆️ 图片并行上传 + **校验滚到首错** + AI 生成 spinner;表单偏长属设计取舍 |
...
| 找服务 | Q&A 问答(服务) | 7.5 | ⬆️ 原审计 4.0 是**重大误报**(有成熟 QASection);**整行可点展开**已加;缺最佳答案/投票 |
| 垂直版块 | 二手 Secondhand | 8.0 | 全平台标杆(图片/评价/直连私信) |
| 垂直版块 | 招聘 Jobs | 7.5 | 无排序、求职模式字段不对称 |
| 垂直版块 | 活动 Events | 7.5 | ⬆️ 移动端详情**改抽屉**(与 Jobs/二手/租房一致);竖列表对时间序活动是合理设计,不强改 Masonry |
| 垂直版块 | 租房 RealEstate | 7.5 | ⬆️ 价格筛选 UI + **切租/售清残留价格(真 bug)**;房型 7 种过细仍可精简 |
...
| 社区沟通 | 通知 Notifications | 7.0 | ⬆️ 管理员历史「加载更多」+ **prefs 跨标签/设备实时同步**;权限横幅本就齐全 |
| 社区沟通 | AI 客服 | 7.0 | ⬆️ 空闲超时 + **失败/超时加「重试」**(干净重发,不污染历史);localStorage 非真漏洞 |
| 账户后台 | 注册登录 Auth | 7.5 | ⬆️ 密码位数统一 + **注册屏加「重发验证邮件」**(收不到邮件不再卡死) |
...
| 账户后台 | 数据面板 Stats | 7.5 | ⬆️ **加「对比同类」**(分类基准 RPC ±%)+ 失败兜底;使用感评 9.0 |
| 账户后台 | 会员 Membership | 7.0 | ⬆️ 原 5.5;**方案A「曝光优先」落地**:L2/L3 = 金标 + 推荐优先展示 + 每月免费置顶(L2 1/L3 3),邀请引擎奖励的黄金真正有价值,空权益缺口已闭合 |
| 账户后台 | 邀请 Referral | 7.0 | ⬆️ 生成失败**改友好文案 + 重试按钮**(原来叫用户"执行修复 SQL") |
| 账户后台 | 我的发布 Services | 7.5 | ⬆️ 编辑图片标识 + **「待审核置顶」标签**;"5类型合并"复核为已共享 Row,不做 |
| 账户后台 | 报价 Inquiries(双侧) | 7.0 | ⬆️ 评价入口 + **手动关闭请求**已加;"接单方无推送"是**误报** |
| 账户后台 | 管理后台 Admin | 7.5 | ⬆️ 认证**批量审核** + **删用户确认列后果 + 修 passcode 误导**;撤销需软删除基建,单列 |
